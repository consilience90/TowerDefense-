...
public class Block extends Rectangle{
	...
	public void draw(Graphics g){
		g.drawImage(Screen.img_info_earth[earthID], x, y, width, height, null);
		g.setColor(new Color(255,150,200));
		g.drawRect(x,y,width,height);
		//분홍색의 격자가 생기게 하였다.
		
		if(buildID != Value.buildNone && earthID != 2){
			g.drawImage(Screen.img_info_tower[buildID + 1], x, y, width, height, null);
	...
	public void physic() {
		...
		if(!aim) {
			...
			if (buildID >= 0 && buildID <= 7) {
				for (int i = 0; i < Screen.mobs.length; i++) {
					//몹이 스폰되면 될 수록 몹 객체가 저장된 mobs[]배열은 점점 더 늘어난다. Screen.mobs.length값은 증가한다.
					if (Screen.mobs[i].inGame) {
						//몹 객체가 죽지 않고 생성되어 있는지를 파악한다.
						if (towerSpace.intersects(Screen.mobs[i])) {
						//해당 towerSquar안에 지정된 몹이 위치하면 공격하게끔 한다.
							aim = true;
							atk = i;
		...
		if(aim) {
			if(aimFrame >= aimTime) {
				Screen.mobs[atk].loseEnemyHP(buildID+1);
				//겨냥된 몹의 체력을 깍는다.
				aimFrame = 0;
	...
	public void getMoney(int mobID) {
		if(!Screen.mobs[mobID].passMob){
			Screen.gold += (Screen.mobs[mobID].mobReward);
		}
